...
public class GmailProvider implements EmailProvider {
    ...
    public GmailProvider(JavaMailSender javaMailSender, RetryRegistry retryRegistry) {
        ...
        Retry retry = retryRegistry.retry("emailService"); //? gets the retry identifier from the properties

        //=> Subscribe to retry events
        retry.getEventPublisher()
                .onRetry(this::logRetryAttempt)   // Hook into retry attempts
                .onSuccess(event -> log.info("Email sent successfully after {} attempts", event.getNumberOfRetryAttempts()))
                .onError(event -> log.error("Failed to send the email after {} attempts", event.getNumberOfRetryAttempts()));
    }
    ...
    */
    private void logRetryAttempt(RetryOnRetryEvent event) {
        log.info("Retry attempt {}/{} for sending email", event.getNumberOfRetryAttempts(), MAX_ATTEMPTS);
    }
    ...
    */
    @Override
    @io.github.resilience4j.retry.annotation.Retry(name = "emailService", fallbackMethod = "handleRetryFailure")
    ...
    public void handleRetryFailure(EmailRequest details, Throwable ex) throws EmailSendException {
        ...
        throw new EmailSendException(String.format("Failed to send email to %s after 3 attempts using Gmail", details.getTo()), ex);
    }
